...
from arbor.tandem.worktree import GitWorktree
...
def _ready_ledger(
    session_dir: Path,
    artifact: Path,
    proposal_id: str = "fp-123",
    *,
    target_relpath: str = "README.md",
    patched_content: str = "Base\nNew docs\n",
    content_hash: str | None = None,
) -> Path:
    patched_path = session_dir / "patched" / f"{proposal_id}.patch"
    ...
    put_role_task_record(
        ...
        RoleTaskRecord(
            ...
            verdict="pass",
            gated_content_hash=content_hash
            or hashlib.sha256(patched_content.encode("utf-8")).hexdigest(),
            gated_target_relpath=target_relpath,
            gated_patched_path=str(patched_path),
        ),
    )
    return patched_path
...
class FakeRunner:
    def __init__(self, responses: list[subprocess.CompletedProcess[str]]) -> None:
        ...
        self.added_content: str | None = None
    ...
    def __call__(
        ...
        *,
        ...
        if list(args)[:3] == ["git", "add", "--"] and cwd is not None:
            self.added_content = (Path(cwd) / args[-1]).read_text(encoding="utf-8")
...
@contextmanager
def _fake_worktree(target_repo_root: Path, worktree: Path) -> Iterator[GitWorktree]:
...
def test_dry_run_default_reports_would_open_without_gh_mutation(tmp_path: Path) -> None:
    ...
    assert "14 bytes" in result.detail
    assert result.body is not None
    assert OPERATOR_DISCLOSURE in result.body
    assert runner.calls == []


def test_enabled_applies_gated_content_in_worktree_branch_before_draft_pr(
    tmp_path: Path,
    ...
    target = tmp_path / "repo"
    target.mkdir()
    (target / "README.md").write_text("RAW HEAD\n", encoding="utf-8")
    artifact = _artifact(target / "docs" / "maintainer-dogfood" / "proposals" / "fp-123.json")
    patched = "Base\nNew docs\n"
    _ready_ledger(session_dir, artifact, patched_content=patched)
    worktree = tmp_path / "isolated-worktree"
    monkeypatch.setattr(
        "arbor.tandem.gh_runner.git_worktree",
        lambda *args, **kwargs: _fake_worktree(target, worktree),
    )
    runner = FakeRunner(
        [
            _completed(stdout=""),
            _completed(code=1, stderr="Not Found (HTTP 404)"),
            _completed(stdout="added"),
            _completed(stdout="committed"),
    ...
    assert runner.added_content == patched
    assert (target / "README.md").read_text(encoding="utf-8") == "RAW HEAD\n"
...
def test_content_hash_mismatch_fails_closed_before_gh_calls(tmp_path: Path) -> None:
    session_dir = tmp_path / "session"
    target = tmp_path / "repo"
    target.mkdir()
    artifact = _artifact(target / "docs" / "maintainer-dogfood" / "proposals" / "fp-123.json")
    _ready_ledger(session_dir, artifact, content_hash="not-the-real-hash")
    runner = FakeRunner([])
    ...
    assert not result.mutated
    assert result.reason_code == "gated-content-divergence"
    assert runner.calls == []
